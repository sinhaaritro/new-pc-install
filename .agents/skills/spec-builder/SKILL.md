...
1. **Draft per lane into the running plan.** Lane B: sections **1 (Goal & Context), 4 (Affected Files & Contracts), 5 (Task DAG)** of `references/implementation-plan-template.md` only. Lane A: all seven sections — 1 Goal & Context, 2 Architectural Decisions & Trade-offs, 3 Risks & Open Questions, 4 Affected Files & Contracts, 5 Task DAG, 6 Verification Commands, 7 Rollback Strategy. Write to `docs/temp/draft-plan.md` and reuse that one running file — never multiple plan files per task. Drafts live in `docs/temp/` (gitignored), never committed.
2. **Validate before presenting** (never show or promote a half-formed plan):
   `python .agents/skills/spec-builder/scripts/promote_spec.py check docs/temp/draft-plan.md --lane <B|A>`
    Exit 0 = the lane's required sections are present, the section-5 Task DAG passes the structure + static-oracle lint, AND the tag lint found no violations; exit 1 prints the missing section numbers and/or the structure violations and/or the tag violations — fix them before continuing.
3. **Maintain the Task DAG state in real time** per `references/todo-checklist-format.md`: state lives on subtasks, `- [ ]` pending → `- [/]` in progress → `- [x]` done. Mark a subtask `[x]` only when its `Verify` exits 0 AND the output matches `Expect`, with evidence recorded in `docs/temp/verify-state.json` — a checkbox without evidence is unmet — and only after the four passes; an impossible subtask is never deleted, it gets `ABANDON: <non-empty reason>`. Revert to `[ ]` when a subtask fails 3 consecutive times (circuit breaker per phase-protocol `build.md`).
...
9. **Deterministic TOC.** Each folder README (+ the docs/ landing README) carries a `## Index` section between `<!-- TOC:START -->`/`<!-- TOC:END -->` markers, regenerated by the engine on every promote/adr/handoff:
...
- Review: the finalized spec — `Phase: 3-Review`, section-5 Task DAG subtasks synced to `- [x]` with re-measured verification evidence (fresh Phase-Gate re-run); no new promotion.
...
- **The handoff record closes the loop:** `docs/temp/handoff.md` is ephemeral; the `Handoff:` header stamp is the durable proof of the split, and `build.md` refuses to build without it.
